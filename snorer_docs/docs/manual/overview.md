...
\end{equation}
$$
is the angular distribution for $\chi\nu$ scattering cross section in lab frame. One can easily verify that Eq. $\eqref{eq:gx}$ satisfy
$$
...
After converting erg to MeV, one can examine that Eq. $\eqref{eq:snv_spectrum}$, after multiplying $\tau_s$, has the unit MeV<sup>−1</sup> cm<sup>−2</sup>.
...
Note that $\delta(r_\nu-d)$ actually carries \[L<sup>−1</sup>\] and cancels the dimension from $c\tau_s$.
